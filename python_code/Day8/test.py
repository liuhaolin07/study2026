...
# 为什么要"模拟"：很多问题（抽奖、排队、误差传播）解析解难求，
# 就用大量随机试验"做实验"，用频率逼近概率——这就是大数定律。
# Day 10-12 的蒙特卡洛求 π 就是本章技巧的大综合。

# ========== 0. 随机种子 seed ==========
# 计算机的"随机"其实是伪随机：给定种子，后面整条序列就完全确定。
# 固定种子 → 每次运行产生同一批"随机"数（结果可复现，写报告/调 bug 必备）
np.random.seed(42)

...
df2 = df.set_index("姓名")
print("\n把姓名设为索引后:")
print(df2.loc["张三"])          # 按标签取一行（loc = label 标签）
print(df2.iloc[0])              # 按位置取第0行（iloc = integer 整数位置）
# ⚠️ 后续细讲的核心区别：loc 按"名字"，切片含两端；iloc 按"座位号"，切片不含末端。
# ⚠️ 链式赋值陷阱：df[df["语文"]>80]["数学"] = 100 可能静默不生效，
# 还会报 SettingWithCopyWarning；改数据永远一步到位：df.loc[行条件, 列] = 值
